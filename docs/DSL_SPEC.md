...
4. **向后兼容**：旧写法（如 `find(text="...")`、`find(loop)`）仍可运行，新语法优先推荐使用。
...
以下旧写法仍可运行，但不建议在新脚本中使用：

| 旧写法 | 推荐新写法 |
|---|---|
| `find(text="签到") { click() }` | `findText("签到") { click() }` |
| `find(color=0xFF5000) { click() }` | `findColor(0xFF5000) { click() }` |
| `find(image="btn.png") { click() }` | `findImage("btn.png") { click() }` |
| `find(location=(x, y))` | `colorAt(x, y)` |
| `find(location=(x, y), color=...) ` | `ifColorAt(x, y, ...)` |
| `find(text="...", loop) { click() }` | `waitForText("...") { click() }` |
| `find(loop) { ... }` | `loop { ... }` |
| `if (find(text="...")) { ... }` | `ifText("...") { ... }` |
| `point p = find(image=...)` | `let p = findImage(...)` |
